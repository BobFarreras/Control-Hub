...
const worker = new Worker("control-hub-system", async (job) => {
  logger.info({ jobId: job.id, jobName: job.name }, "processing system job");
  return processSystemJob(job);
}, { connection: { host: connection.hostname, port: Number(connection.port || 6379), password: connection.password || undefined }, concurrency: 4 });
