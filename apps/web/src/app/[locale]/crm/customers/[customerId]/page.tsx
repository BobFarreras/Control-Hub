...
async function loadCustomer(customerId: string) { const response = await apiFetch(`/api/v1/crm/customers/${customerId}`); if (response.status === 404) notFound(); if (!response.ok) throw new Error("CRM_LOAD_ERROR"); return (await response.json()).customer; }
