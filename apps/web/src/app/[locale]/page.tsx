...
export default async function DashboardPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <div className="app-shell">
    ...
    <div className="workspace">
      <PageTopbar eyebrow={t.dashboard.eyebrow} title={t.dashboard.title} description={t.dashboard.description} themeLabel={t.header.theme} actions={<><span className="health"><i />{t.header.healthy}</span><div className="locale-switch" aria-label={t.header.language}>{locales.map((locale) => <Link className={locale === localeParam ? "selected" : ""} href={`/${locale}`} key={locale}>{locale.toUpperCase()}</Link>)}</div></>} />
      <main className="compact-main">
        <section className="dashboard-metrics" aria-label="Metrics">{metrics.map(([label, value, Icon]) => <article key={label}><div className="metric-icon"><Icon size={17} /></div><span>{label}</span><strong>{value}</strong></article>)}</section>
