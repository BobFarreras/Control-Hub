...
type Loaded = { host: ObservedHost | null; instanceNames: Record<string, string> };

async function load(hostId: string): Promise<Loaded> {
  try {
    const [inventoryResponse, integrationsResponse] = await Promise.all([
      apiFetch("/api/v1/infrastructure/inventory"),
      apiFetch("/api/v1/integrations")
    ]);
    if (!inventoryResponse.ok) return { host: null, instanceNames: {} };
    ...
    return { host: null, instanceNames: {} };
  }
}
...
export default async function HostPage({ params }: { params: Promise<{ locale: string; hostId: string }> }) {
  ...
  const { host, instanceNames } = await load(hostId);
