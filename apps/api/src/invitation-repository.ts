...
export async function createInvitation(database: DatabaseClient, context: TenantContext, input: { email: string; role: InvitationRole; expiresAt: Date }) {
  const token = randomBytes(32).toString("base64url");
  const invitationId = randomUUID();
  const email = input.email.trim().toLowerCase();
  try {
    await withTenant(database, context.tenantId, (tx) => tx`
    ...
    if (typeof error === "object" && error && "code" in error && error.code === "23505") throw new InvitationError("INVITATION_ALREADY_PENDING");
    throw error;
  }
...
export async function listInvitations(database: DatabaseClient, context: TenantContext) {
  return withTenant(database, context.tenantId, (tx) => tx`
...
export async function revokeInvitation(database: DatabaseClient, context: TenantContext, invitationId: string) {
  const rows = await withTenant(database, context.tenantId, (tx) => tx`
...
export async function lookupInvitation(database: DatabaseClient, token: string) {
  ...
  const rows = await database<{ id: string; tenant_id: string; tenant_name: string; email: string; role_code: InvitationRole; expires_at: Date }[]>`
  ...
  return { id: invitation.id, tenantId: invitation.tenant_id, tenantName: invitation.tenant_name, email: invitation.email, role: invitation.role_code, expiresAt: invitation.expires_at };
}
