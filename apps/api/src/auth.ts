...
export function createAuth(environment: ApiEnvironment, options: { allowSignUp?: boolean } = {}) {
  ...
  const auth = betterAuth({
    ...
    emailAndPassword: {
      ...
      sendResetPassword: async ({ user, url }) => void sendMail({ to: user.email, subject: "Control Hub - Restablir contrasenya", text: url })
    },
    emailVerification: {
      sendOnSignIn: true,
      autoSignInAfterVerification: false,
      expiresIn: 3600,
      sendVerificationEmail: async ({ user, url }) => void sendMail({ to: user.email, subject: "Control Hub - Verificar correu", text: url })
    },
    ...
    plugins: [
      twoFactor({ issuer: "Control Hub", backupCodeOptions: { amount: 10, length: 12 }, totpOptions: { period: 30, digits: 6 } }),
      passkey({ rpID: environment.WEBAUTHN_RP_ID, rpName: "Control Hub", origin: environment.WEBAUTHN_ORIGIN, authenticatorSelection: { userVerification: "required", residentKey: "preferred" } })
    ]
  });
