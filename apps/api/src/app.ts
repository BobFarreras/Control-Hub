...
export function buildApp(options: BuildAppOptions) {
  ...
  if (options.auth) {
    ...
    app.post<{ Body: { email: string; role: InvitationRole; locale?: "ca" | "es" | "en" } }>(
      ...
      async (request, reply) => {
        ...
        const url = new URL(`/${locale}/accept-invitation`, options.appOrigin);
        url.searchParams.set("token", invitation.token);
        try {
          await options.sendMail({ to: invitation.email, ...invitationMessage(locale, url.toString()) });
        } catch (error) {
  ...
  app.post<{ Body: { token: string; name: string; password: string } }>(
    ...
    async (request, reply) => {
      ...
      const registration = await options.invitationAuth.api.signUpEmail({
