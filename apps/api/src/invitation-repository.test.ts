...
suite("member invitations", () => {
  ...
  it("isolates, resolves and consumes a one-use token", async () => {
    const created = await createInvitation(database, context, {
      email: `invitee-${inviteeId}@test.local`,
      role: "technical",
      expiresAt: new Date(Date.now() + 60_000)
    });
    const stored = await admin<
      { token_hash: string }[]
    >`select token_hash from member_invitations where id = ${created.id}`;
    expect(stored[0]?.token_hash).toBe(hashInvitationToken(created.token));
