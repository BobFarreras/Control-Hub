...
const environment = parseApiEnvironment({ ...process.env, DATABASE_URL: process.env.MIGRATION_DATABASE_URL ?? process.env.DATABASE_URL });
...
if (!email || !password || password.length < 12 || !name || !tenantName || !tenantSlug?.match(/^[a-z0-9][a-z0-9-]{1,61}[a-z0-9]$/)) {
  throw new Error("BOOTSTRAP_OWNER_EMAIL, BOOTSTRAP_OWNER_PASSWORD (12+), BOOTSTRAP_OWNER_NAME, BOOTSTRAP_TENANT_NAME and a valid BOOTSTRAP_TENANT_SLUG are required");
}
...
try {
  ...
  const result = existingUser[0] ? { user: existingUser[0] } : await auth.api.signUpEmail({ body: { email, password, name } });
  ...
  await database.begin(async (transaction) => {
    ...
    for (const roleCode of Object.keys(rolePermissions) as RoleCode[]) {
      ...
      for (const permission of rolePermissions[roleCode]) await transaction`insert into role_permissions (role_id, permission_code) values (${roleId}, ${permission})`;
      if (roleCode === "owner") await transaction`insert into membership_roles (membership_id, role_id) values (${membershipId}, ${roleId})`;
    }
  });
  await withTenant(database, tenantId, async (transaction) => transaction`
  ...
  `);
  await auth.close();
  console.info("Control Hub bootstrap completed. Verify the Owner email and enable MFA before using protected modules.");
} finally {
