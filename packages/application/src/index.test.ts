...
const context: TenantContext = { tenantId: "tenant-a", membershipId: "member-a", userId: "user-a", roles: ["owner"], permissions: ["leads:manage"], mfaEnabled: true };
const lead: LeadRecord = { id: "lead-a", name: "Avant", companyName: null, email: null, phone: null, source: "manual", status: "new", priority: "normal", ownerMembershipId: null, convertedCustomerId: null, createdAt: new Date(), updatedAt: new Date() };
...
function repository(overrides: Partial<CrmRepository> = {}): CrmRepository {
  return {
    ...
    listCustomers: vi.fn(), createLead: vi.fn().mockResolvedValue(lead), transitionLead: vi.fn().mockResolvedValue({ ...lead, status: "contacted" }), convertLead: vi.fn(),
    getCustomer: vi.fn(), addContact: vi.fn(), addNote: vi.fn(), addTask: vi.fn(), completeTask: vi.fn(), commercialSummary: vi.fn(), ...overrides
  };
}
...
describe("CrmService", () => {
  it("normalizes duplicate keys before persistence", async () => {
    ...
    await service.createLead(context, { name: " Àvant ", email: " SALES@EXAMPLE.COM ", phone: "+34 600 123 123", source: " manual ", priority: "normal" });
    expect(adapter.createLead).toHaveBeenCalledWith(context, expect.objectContaining({ name: "Àvant", source: "manual", normalizedName: "avant", normalizedEmail: "sales@example.com", normalizedPhone: "+34600123123" }));
  });
