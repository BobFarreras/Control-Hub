...
const defaults = (tableId: string): TablePreference => ({
  tableId,
  columnOrder: [],
  hiddenColumns: [],
  columnWidths: {},
...
export async function getTablePreference(database: DatabaseClient, context: TenantContext, tableId: string) {
  return withTenant(database, context.tenantId, async (tx) => {
    const rows = await tx<
      TablePreference[]
    >`select table_id as "tableId", column_order as "columnOrder", hidden_columns as "hiddenColumns", column_widths as "columnWidths", page_size as "pageSize" from user_table_preferences where tenant_id = ${context.tenantId} and user_id = ${context.userId} and table_id = ${tableId}`;
    return rows[0] ?? defaults(tableId);
  });
}
