...
export class PostgresCompanySubscriptionRepository implements CompanySubscriptionRepository {
  ...
  list(context: TenantContext) {
    return withTenant(this.database, context.tenantId, (tx) =>
      tx.unsafe<CompanySubscriptionRecord[]>(
        `${select} where tenant_id = $1 order by coalesce(renewal_at, 'infinity'), service_name`,
        [context.tenantId]
      )
    );
  }
  async create(context: TenantContext, input: CreateCompanySubscription) {
    try {
      ...
      if (typeof error === "object" && error && "code" in error && error.code === "23505")
        throw new CompanySubscriptionError("DUPLICATE_SUBSCRIPTION");
      throw error;
    }
  }
  updateStatus(context: TenantContext, id: string, status: CompanySubscriptionStatus) {
    return withTenant(this.database, context.tenantId, async (tx) => {
      ...
      if (!rows[0]) throw new CompanySubscriptionError("COMPANY_SUBSCRIPTION_NOT_FOUND");
      return rows[0];
    });
  }
}
