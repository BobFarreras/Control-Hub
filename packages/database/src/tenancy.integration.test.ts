...
suite("tenant RLS", () => {
  let database: DatabaseClient; let adminDatabase: DatabaseClient;
  ...
  beforeAll(async () => {
    ...
    await withTenant(database, tenantA, (tx) => tx`insert into tenant_settings (tenant_id, brand_name) values (${tenantA}, 'A')`);
    await withTenant(database, tenantB, (tx) => tx`insert into tenant_settings (tenant_id, brand_name) values (${tenantB}, 'B')`);
  });
  afterAll(async () => {
  ...
  it("does not expose another tenant when its identifier is manipulated", async () => {
    const rows = await withTenant(database, tenantA, (tx) => tx`select brand_name from tenant_settings where tenant_id = ${tenantB}`);
    expect(rows).toHaveLength(0);
  });
  it("keeps audit events append-only", async () => {
    const auditId = randomUUID();
    await expect(withTenant(database, tenantA, async (tx) => {
