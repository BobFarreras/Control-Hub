...
**El seguent pas es actualitzar la VPS a la `v0.4.4`**, que ja esta publicada. Aquella maquina
corre amb la `v0.4.2` i cal actualitzar-la per tenir tots els moduls activats i la detecció
automàtica de n8n.
...
**La `v0.4.4` inclou millores importants per a la VPS:**
...
- **Detecció automàtica de n8n**: Tant `install.sh` com `update.sh` detecten si hi ha un contenidor n8n a la mateixa màquina i l'afegeixen automàticament a `CONNECTOR_INTERNAL_ALLOWLIST`. Això permet que els connectors puguin connectar amb n8n sense configuració manual.
